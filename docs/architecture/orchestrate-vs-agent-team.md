...
| | orchestrate.sh | Agent Team |
|---|---|---|
| **작업 중 확인** | **Terminal 탭 (LiveTerminalPanel)**: WebSocket으로 JSONL 대화 스트림을 실시간 수신하여 워커의 tool call, 사고 과정을 UI에서 확인 가능. 기존 로그 파일 tail도 여전히 가능 | **split-pane에서 해당 agent 터미널에 직접 진입**하여 실시간 확인 |
| **개입** | `stopped` 시그널로 중단만 가능 | 터미널에 타이핑하여 즉시 방향 전환/질문 가능 |
| **진행 상황** | Terminal 탭에서 현재 활동(tool call, 파일 읽기/수정) 실시간 확인 + 시그널 기반 상태 전이 | Shift+Down으로 각 teammate 순회하며 현재 상태 확인 |

**2026-03-31 업데이트**: LiveTerminalPanel 구현으로 블랙박스 문제가 **부분적으로 해소**됨. 워커의 JSONL 스트림(`--output-format stream-json`)을 WebSocket을 통해 프론트엔드에 실시간 전달하여, 어떤 파일을 읽고 있고 어떤 tool을 호출하고 있는지 확인 가능. 다만 Agent Team처럼 **타이핑으로 즉시 개입**하는 것은 아직 불가 — 이 부분은 여전히 gap으로 남아 있음.
...
| | orchestrate.sh | Agent Team |
|---|---|---|
| **dispatch 오버헤드** | 셸 루프 polling → 의존성/충돌 체크 → context-builder 프롬프트 조립 → 프로세스 spawn → 시그널 파일 I/O. **2026-03-31 개선**: dispatch sleep 2→0.3초, idle polling 30→5초로 단축 | Claude 판단 → 하네스가 즉시 API 세션 생성. 오버헤드 거의 0 |
...
### 3.9 데이터 정확성 (외부 오케스트레이션 고유 문제)

orchestrate.sh는 상태를 파일(frontmatter)과 프론트엔드(React store) 양쪽에서 관리하므로, **상태 불일치** 문제가 발생할 수 있다. Agent Team은 상태가 내부적으로 단일 소스에서 관리되어 이 문제가 없음.

**실제 사례 (2026-03-31)**:
- 프론트엔드의 `tasksStore.ts`에서 `failed` 상태를 누락하여, 실패한 태스크가 UI에서 표시되지 않는 버그 발견/수정
- `context-builder.sh`가 worktree 디렉토리 존재 여부를 확인하지 않아 크래시 발생 → 디렉토리 체크 추가로 수정

이런 유형의 버그는 외부 오케스트레이션의 **구조적 비용** — 셸 스크립트, 프론트엔드, 태스크 파일 간 동기화를 수동으로 보장해야 하므로 누락이 발생하기 쉬움.
...
#### (C) 워커 실시간 관찰 (Live Observability) — 부분 구현 완료
**이전**: 워커가 시작되면 완료/실패 시그널이 올 때까지 **블랙박스**. 로그 파일을 `tail -f`로 볼 수는 있지만, Claude의 사고 과정이나 현재 어떤 파일을 수정 중인지 실시간으로 알 수 없었음.

**2026-03-31 구현**: `LiveTerminalPanel` 컴포넌트를 통해 프론트엔드 Terminal 탭에서 워커의 JSONL 대화 스트림을 WebSocket으로 실시간 수신. tool call, 파일 읽기/수정 등의 활동을 실시간으로 확인 가능해짐.
...
**남은 gap**: 실시간 **관찰**은 가능해졌지만, 실시간 **개입**(타이핑으로 방향 전환)은 아직 미구현. 이 부분은 5.1-B (실시간 방향 전환)와 연계.

**기존 제안 (참고)**: iTerm 모드에서 각 워커 탭의 실시간 상태를 대시보드로 제공
```
┌─ orchestrate.sh 대시보드 ──────────────────────────┐
│                                                    │
...
| 순위 | 개선 항목 | 효과 | 난이도 | 상태 |
|------|----------|------|--------|------|
| 1 | **큰 태스크 + 워커 자율 분해 허용 (5.1-D)** | 체감 속도 대폭 개선, Agent Team의 속도를 안전장치 유지하며 확보 | 중 | 미착수 |
| 2 | **워커 실시간 관찰 대시보드 (5.1-C)** | 블랙박스 해소, 잘못된 방향 조기 발견 → 비용/시간 절약 | 중 | **부분 완료** — LiveTerminalPanel (WebSocket JSONL 스트림) 구현. 실시간 개입은 미구현 |
| 3 | 복잡도 기반 모델/병렬 자동 조절 (5.1-E) | 비용 30~50% 절감 가능 | 중 | 미착수 |
| 4 | 리뷰 피드백 구조화 (5.2-F 강화) | 반복 실패 패턴 감지 → 태스크 품질 향상 | 중 | 미착수 |
| 5 | 워커 내부 서브에이전트 활용 안내 (5.3) | 복잡 태스크 성공률 향상 | 낮 | 미착수 |
| 6 | 실시간 방향 전환 (5.1-B) | 비용 절약, 실패 감소 | 높 | 미착수 |
| 7 | 워커 간 메시지 채널 (5.1-A) | 인터페이스 변경 시 연계 | 낮 | 미착수 |
| — | dispatch/polling 속도 개선 (3.8 관련) | 디스패치 반응성 개선 | 낮 | **완료** — dispatch sleep 2→0.3s, idle polling 30→5s |
| — | 데이터 정확성 버그 수정 (3.9) | 상태 불일치 해소 | 낮 | **완료** — failed 상태 누락 수정, context-builder 크래시 수정 |
