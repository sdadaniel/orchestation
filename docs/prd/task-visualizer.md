...
- Task 상태(`pending`, `in_progress`, `in_review`, `done`)를 바 색상으로 구분
...
- **Task/Sprint CRUD**: 웹 UI에서 md 파일 생성·수정·삭제, 상태 변경 (pending → in_progress → in_review → done)
...
- Task = 수평 바 (상태별 색상)
- 상태별 색상 구분 (pending: 회색, in_progress: 파랑, in_review: 주황, done: 초록)
