...
export function computeDAGLayout(
  ...
  for (let d = 0; d <= Math.min(maxDepth, MAX_PENDING_QUEUES - 1); d++) {
    pendingByDepth.push(
      allPending
        ...
        .sort(
          (a, b) =>
            priWeight(a.priority) - priWeight(b.priority) ||
  ...
  if (allFailed.length > 0)
    sections.push({
      key: "failed",
      label: "FAILED",
      items: allFailed.slice(0, maxParallel),
      color: "#ef4444",
      extra: Math.max(0, allFailed.length - maxParallel),
    });
  ...
  if (ghostCount > 0) {
    const ghostH =
      SECTION_HEADER_H + maxParallel * (NODE_H + ROW_GAP) + ROW_GAP;
    sectionLayouts.push({
      key: "ghost",
      label: "PENDING",
      x: sectionX,
  ...
  const allXR = nodes
    .map((n) => n.x + NODE_W)
    .concat(sectionLayouts.map((s) => s.x + s.w));
  const allY = nodes.map((n) => n.y).concat(sectionLayouts.map((s) => s.y));
  const allYB = nodes
    .map((n) => n.y + NODE_H)
    .concat(sectionLayouts.map((s) => s.y + s.h));
  const bounds =
    nodes.length === 0
      ? { minX: 0, minY: 0, maxX: sectionX, maxY: 400 }
      : {
          minX: Math.min(...allX) - CANVAS_PAD,
          minY: Math.min(...allY) - CANVAS_PAD,
          maxX: Math.max(...allXR) + CANVAS_PAD,
          maxY: Math.max(...allYB) + CANVAS_PAD,
        };
  ...
  const computeGroup = (keys: Set<string>, includeGhost: boolean) => {
    ...
    return {
      ...
      w:
        Math.max(...matched.map((s) => s.x + s.w)) -
        Math.min(...matched.map((s) => s.x)) +
        24,
      h:
        Math.max(...matched.map((s) => s.y + s.h)) -
        Math.min(...matched.map((s) => s.y)) +
        60,
    };
  };
  ...
  const pendingKeys = new Set(
    sections
      .filter((s) => s.key.startsWith("pending-") || s.key === "queue")
      .map((s) => s.key),
  );
  const topGroups = [
    {
      label: "PENDING",
      color: "#eab308",
      box: computeGroup(pendingKeys, true),
    },
    {
      label: "FAILED",
      color: "#ef4444",
      box: computeGroup(new Set(["failed"]), false),
    },
    {
      label: "IN PROGRESS",
      color: "#3b82f6",
      box: computeGroup(new Set(["current"]), false),
    },
    {
      label: "DONE",
      color: "#22c55e",
      box: computeGroup(new Set(["done"]), false),
    },
  ].filter(
    (
      g,
  ...
  let ghostEdge: { x1: number; y1: number; x2: number; y2: number } | null =
    null;
...
export default function DAGCanvas({
  ...
  useEffect(() => {
    fetch("/api/settings")
      .then((r) => {
        ...
        return r.json() as Promise<{ maxParallel?: number }>;
      })
      .then((d) => {
  ...
  const apply = useCallback(() => {
    if (gRef.current)
      gRef.current.setAttribute(
        "transform",
        `translate(${tf.current.x},${tf.current.y}) scale(${tf.current.scale})`,
      );
  }, []);
  ...
  const fit = useCallback(() => {
    ...
    tf.current = {
      x: r.width / 2 - ((b.minX + b.maxX) / 2) * s,
      y: r.height / 2 - ((b.minY + b.maxY) / 2) * s,
      scale: s,
    };
    apply();
    kick((n) => n + 1);
  ...
  const onWh = useCallback(
    (e: React.WheelEvent) => {
      ...
      const t = tf.current,
        cx = e.clientX - r.left,
        cy = e.clientY - r.top,
        ns = Math.min(Math.max(t.scale * (e.deltaY < 0 ? 1.1 : 0.9), 0.1), 3),
        ra = ns / t.scale;
      tf.current = {
        x: cx - (cx - t.x) * ra,
        y: cy - (cy - t.y) * ra,
  ...
  const bc = (s: string, nu: boolean) =>
    nu
      ? "#facc15"
      : s === "in_progress" || s === "reviewing"
        ? "#3b82f6"
        : s === "done"
          ? "#22c55e"
          : s === "rejected"
    ...
    >
      <div className="absolute top-2 right-2 z-20 flex items-center gap-1 bg-card/80 backdrop-blur border border-border rounded-md px-2 py-1">
        ...
        <span className="text-[10px] text-muted-foreground w-8 text-center">
          {Math.round(tf.current.scale * 100)}%
        </span>
      </div>
      ...
      <svg
        ref={svgRef}
        className="dag-canvas w-full h-full rounded-lg border border-border bg-background"
        onMouseDown={onMD}
        onMouseMove={onMM}
        onMouseUp={onMU}
        onMouseLeave={onMU}
            ...
            return (
              <g key={k}>
                ...
                <path
                  ...
                  opacity={h ? 1 : 0.5}
                  markerEnd={h ? "url(#dag-arrow-hover)" : "url(#dag-arrow)"}
            ...
            >
              ...
              >
                <div className="flex items-center gap-1.5">
                  ...
                  <span
                    className={cn(
                      "text-[9px] px-1 py-0.5 rounded border font-medium ml-auto shrink-0",
                      PRIORITY_COLORS[n.req.priority],
                    )}
                  >
                    {n.req.priority}
                  </span>
                  <button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      if (!pan.current?.moved) onClickItem(n.req);
                    }}
          ...
          {layout.ghostBox && (
            <g>
              <rect
                x={layout.ghostBox.x}
                y={layout.ghostBox.y}
                width={layout.ghostBox.w}
                height={layout.ghostBox.h}
                rx={8}
                fill="var(--muted)"
                fillOpacity={0.3}
                stroke="#71717a"
