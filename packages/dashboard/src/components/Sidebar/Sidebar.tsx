...
function CollapsedNavItem({
  ...
  return (
    ...
    >
      {icon}
      {badge != null && badge > 0 && (
        <span className="absolute -top-0.5 -right-0.5 min-w-[14px] h-3.5 px-0.5 rounded-full bg-red-500 text-white text-[8px] flex items-center justify-center font-bold leading-none">
          {badge > 9 ? "9+" : badge}
        </span>
      )}
    </Link>
...
const Sidebar = () => {
  ...
  return (
    <div
      className="ide-sidebar flex flex-col h-full transition-[width] duration-200"
      style={{
      ...
      {collapsed ? (
        ...
        >
          ...
          <CollapsedNavItem
            ...
            badge={tasksSummary.active.length > 0 ? tasksSummary.active.length : undefined}
          />
          <CollapsedNavItem
            href="/notices"
            ...
            badge={noticeSummary.unreadCount > 0 ? noticeSummary.unreadCount : undefined}
          />
        </div>
      ) : (
        <div
