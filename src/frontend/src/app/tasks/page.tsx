...
function TasksPageInner() {
  ...
  const sorted = useMemo(() => {
    ...
    switch (sortKey) {
      ...
      case "priority": {
        ...
        return items.sort(
          (a, b) => w(a.priority) - w(b.priority) || byOrder(a, b),
        );
      }
      case "id":
  ...
  const grouped = useMemo(() => {
    const byStatus = (status: string) =>
      sorted.filter((r) => r.status === status);
    return {
      ...
      {showFilters && (
        <div className="relative">
          ...
          <Input
            ...
            className="bg-muted/50 rounded-lg pl-9 pr-3 py-2 placeholder:text-muted-foreground/50"
          />
        </div>
      )}
            ...
            {(["all", "high", "medium", "low"] as const).map((p) => (
              <button
                ...
                className={cn(
                  ...
                  priorityFilter === p && "active",
                  priorityFilter === p && p !== "all" && PRIORITY_COLORS[p],
                )}
              >
              ...
              {taskScopes.length > 0 && (
                <div className="rounded-lg border border-blue-500/20 bg-blue-500/5 px-3 py-2">
                  ...
                  <div className="space-y-2">
                    {taskScopes.map((t) => (
                      <div key={t.taskId}>
                        <div className="text-[10px] font-medium text-blue-300 mb-1">
                          {t.taskId}{" "}
                          <span className="text-blue-500/50 font-normal">
                            {t.title}
                          </span>
                        </div>
                        <div className="flex flex-wrap gap-1">
                          {[...t.groups.entries()].map(([prefix, paths]) => (
                            <span
                              key={prefix}
                              className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-blue-500/10 text-blue-300/80 border border-blue-500/15"
                            >
                              {prefix}
                              {paths.length > 1
                                ? ` (${paths.length})`
                                : paths[0] !== prefix
                                  ? `/${paths[0].slice(prefix.length + 1)}`
                                  : ""}
                            </span>
                          ))}
                        </div>
          ...
          {depChainGroups.map((group, gi) =>
            group.isChain ? (
              <ChainGroup
                ...
                isFirst={gi === 0}
                isLast={gi === depChainGroups.length - 1}
              />
            ) : (
