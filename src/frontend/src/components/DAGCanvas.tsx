...
export default function DAGCanvas({ requests, tasks, onClickItem }: { requests: RequestItem[]; tasks: WaterfallTask[]; onClickItem: (req: RequestItem) => void }) {
  ...
  useEffect(() => { fetch("/api/settings").then((r) => { if (!r.ok) throw new Error("fetch failed"); return r.json() as Promise<{ maxParallel?: number }>; }).then((d) => { if (d.maxParallel) setMaxParallel(d.maxParallel); }).catch(() => {}); }, []);
