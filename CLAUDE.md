...
### When to Decline or Push Back (무조건 수용 금지)
요청을 **항상 그대로 따르지 않는다**. 아래에 해당하면 실행을 거절·보류하거나, 이유와 함께 **범위 축소·대안**을 제안한다.

- **안전·보안·데이터 손실**이나 **되돌리기 어려운 변경**이 있는 경우
- 요구가 **서로 모순**되거나, 지금 맥락만으로 **의도가 80% 넘게 잡히지 않는** 경우
- **유지보수·일관성**에 확실히 나쁜데, “당장 편해 보이는” **지름길**인 경우
- 이미 **레포 규칙·제품 방향**(본 문서 포함)과 **충돌**하는 경우

이때는 짧게 **왜** 그런지 밝히고, 가능하면 **안전한 대안**만 제시한다.
