...
setup_context_filter() {
  ...
  # worktree 디렉토리 존재 확인 — 없으면 생략 (set -e 안전)
  if [ ! -d "$worktree_path" ]; then
    echo "⚠️ context-builder: worktree 미존재 ($worktree_path) — .claudeignore 생략" >&2
    return 0
  fi

  local ignore_file="$worktree_path/.claudeignore"

  # 기본 제외 패턴 (실패해도 태스크 실행은 계속)
  cat > "$ignore_file" <<'IGNORE' || { echo "⚠️ .claudeignore 생성 실패 — 생략" >&2; return 0; }
...
IGNORE
  ...
  # srcPaths 기반 필터: srcPaths에 포함되지 않는 최상위 디렉토리 제외
  if [ -n "${SRC_PATHS:-}" ]; then
    {
      echo ""
      echo "# srcPaths 외 소스 디렉토리 제외 (config.json srcPaths 기반)"
      local _src_dir
      for _src_dir in "$worktree_path"/*/; do
        [ -d "$_src_dir" ] || continue
        local _dirname
        _dirname=$(basename "$_src_dir")
        case "$_dirname" in
          .git|.orchestration|node_modules|output|docs|scripts|template) continue ;;
        esac
        local _in_src=false
        IFS=',' read -ra _src_arr <<< "$SRC_PATHS"
        for _sp in "${_src_arr[@]}"; do
          _sp=$(echo "$_sp" | sed 's|/$||')
          if [ "$_dirname" = "$_sp" ] || echo "$_sp" | grep -q "^${_dirname}/"; then
            _in_src=true
            break
          fi
        done
        if [ "$_in_src" = false ]; then
          echo "${_dirname}/"
        fi
      done
  ...
  if [ -n "$done_files" ]; then
    {
      ...
      while IFS= read -r fname; do
        [ -z "$fname" ] && continue
        echo "docs/task/${fname}"
        echo "docs/requests/${fname}"
      done <<< "$done_files"
    } >> "$ignore_file" 2>/dev/null || true
  fi
}
