...
status: done
branch: task/task-336
worktree: ../repo-wt-task-336
priority: medium
mode: night
created: 2026-04-03
updated: 2026-04-03 02:19
depends_on: []
scope:
...
export const logger = {
  debug: (msg: string, ...args: unknown[]) => shouldLog("debug") && console.debug(`[DEBUG] ${msg}`, ...args),
  info:  (msg: string, ...args: unknown[]) => shouldLog("info")  && console.info(`[INFO] ${msg}`, ...args),
  warn:  (msg: string, ...args: unknown[]) => shouldLog("warn")  && console.warn(`[WARN] ${msg}`, ...args),
  error: (msg: string, ...args: unknown[]) => shouldLog("error") && console.error(`[ERROR] ${msg}`, ...args),
};
```
